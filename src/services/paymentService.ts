...
interface PaymentProvider {
  createPayment(amount: number, currency: string, orderId: string): Promise<any>;
...
class MockPaymentProvider implements PaymentProvider {
  async createPayment(amount: number, currency: string, orderId: string): Promise<any> {
...
export class PaymentService {
  ...
  async processPayment(orderId: string, paymentId: string, signature?: string): Promise<boolean> {
    try {
      const order = await Order.findOne({ orderId });
      if (!order) {
        throw new Error('Order not found');
