...
class SMSService {
  ...
  async sendOrderConfirmation(phone: string, orderId: string): Promise<boolean> {
    try {
      if (process.env.NODE_ENV === 'development') {
        console.log(`📱 Order confirmation for ${phone}: Order ${orderId} confirmed`);
        return true;
      }
      ...
      const message = `Your Shoppers9 order ${orderId} has been confirmed! Track your order in the app. Thank you for shopping with us.`;
  ...
  async sendOrderUpdate(phone: string, orderId: string, status: string): Promise<boolean> {
    try {
      if (process.env.NODE_ENV === 'development') {
        console.log(`📱 Order update for ${phone}: Order ${orderId} is ${status}`);
        return true;
      }
      ...
      switch (status.toLowerCase()) {
        case 'shipped':
          message = `Great news! Your Shoppers9 order ${orderId} has been shipped and is on its way to you.`;
          break;
        case 'delivered':
          message = `Your Shoppers9 order ${orderId} has been delivered! We hope you love your purchase.`;
          break;
        case 'cancelled':
          message = `Your Shoppers9 order ${orderId} has been cancelled. Refund will be processed within 3-5 business days.`;
          break;
        default:
          message = `Your Shoppers9 order ${orderId} status has been updated to: ${status}.`;
      }
