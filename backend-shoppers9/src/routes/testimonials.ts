...
router.get('/featured', async (req, res, next) => {
  try {
    ...
    const response = await axios.get(`${ADMIN_API_URL}/public/testimonials/featured?limit=${limit}`);
...
router.get('/category/:category', async (req, res, next) => {
  try {
    ...
    const response = await axios.get(`${ADMIN_API_URL}/public/testimonials/category/${category}?limit=${limit}`);
    res.json(response.data);
  } catch (error) {
...
router.get('/rating/:minRating?', async (req, res, next) => {
  try {
    ...
    const limit = req.query.limit || 10;
    const response = await axios.get(`${ADMIN_API_URL}/public/testimonials/rating?minRating=${minRating}&limit=${limit}`);
    res.json(response.data);
  } catch (error) {
...
router.get('/product/:productId', async (req, res, next) => {
  try {
    ...
    const response = await axios.get(`${ADMIN_API_URL}/public/testimonials/product/${productId}`);
    res.json(response.data);
  } catch (error) {
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const response = await axios.get(`${ADMIN_API_URL}/public/testimonials?page=${page}&limit=${limit}&isActive=true`);
    res.json(response.data);
  } catch (error) {
