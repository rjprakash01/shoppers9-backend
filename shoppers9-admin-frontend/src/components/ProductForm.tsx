...
const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const fetchAvailableFilters = async (categoryId: string) => {
    try {
      ...
      // Only fetch filters for level 2 and 3 categories
      if (selectedCategory.level === 1) {
        console.log('🔍 ProductForm: Level 1 category selected, no filters available');
        setAvailableFilters([]);
        setAvailableFilterOptions({});
        return;
      }
      ...
      // Check if it's a 400 error (filters can only be assigned to level 2/3 categories)
      if (error.response && error.response.status === 400) {
        console.log('🔍 ProductForm: Level 1 category - no filters available');
        setAvailableFilters([]);
        setAvailableFilterOptions({});
      } else {
        console.error('❌ ProductForm: Unexpected error:', error.message);
        setAvailableFilters([]);
        setAvailableFilterOptions({});
      }
    }
